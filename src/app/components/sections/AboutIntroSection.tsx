          ...
          <Card className={`border-2 ${config.cardBorderStatic} ${config.surfaceBg} shadow-lg backdrop-blur`}>
            <CardContent className="px-6 py-10 space-y-10">
              <div className="grid gap-10 lg:grid-cols-[220px_1fr_240px] items-start text-left">
                ...
                <div className="flex flex-wrap justify-center gap-2 lg:justify-start lg:content-start">
                  {ABOUT_CONTENT.socials.map((social) => (
                    <button
                      key={social.label}
                      type="button"
                      onClick={() => window.open(social.url, '_blank')}
                      className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-medium text-white shadow transition hover:brightness-110 cursor-pointer ${staticButtonBg} ${config.surfaceBorder}`}
                    >
                        {social.icon === 'x' ? (
